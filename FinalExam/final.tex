...
\item \textbf{How do you propose verifying the installibility of your game?}

You might be able to verify the installibility of your game by having your group members test installing it, as well as having other people not just from your software class, but from other disciplines and programs try to install it too. Then they can give you a score out of whatever metric you decide to give you a more accurate assessment of how your program;s installibility is and can give you feedback on how to improve it. 
...
I would estimate that the collective hours that the team would spend on verification would be two or so hours per person a week, to get together and make sure that what everyone is working on is indeed what they should be and that everyone is sticking to the plan that was made in the last question to reach the milestones in time. 
...
Yes the oracle problem might be a concern for this as we would not be exactly sure how to implement the AI to be appropriate for a human to play against. This could be tested by player playing against it in order to adjust the play of the AI. 
...
SIZE = 3 \textit{\# The size of the game board is always 3x3} ~\\
...
\wss{Return the cellT that corresponds to the current turn}\\
...
\begin{itemize}
\item output: $(if\_winner(c[0]) \Rightarrow True | if\_winner(c[1]) \Rightarrow True | count(c) = 9 \Rightarrow True $
\textit{\# Where c[0] is X and c[1] is O}~\\\

\wss{Returns true if X or O wins, or if there are no more moves remaining}\\
...
diagonal\_win$(c, b) \equiv \exists (j : \mathbb{N} | 0 \leq j < \mbox{SIZE} : b[0, 0] = b[1, 1] = b[2, 2] = c)$
